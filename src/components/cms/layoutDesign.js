			...
			<Row className="pt-1">
							...
							<ButtonGroup>
								...
								<DropdownButton as={ButtonGroup} title="Actions" variant="light">
									<Dropdown.Item eventKey="1">
										<i className="fa fa-cloud-upload" /> Publish
									</Dropdown.Item>
									<Dropdown.Item eventKey="2">
										<i className="fa fa-thumbs-o-up" /> Activate
									</Dropdown.Item>
									<Dropdown.Item eventKey="3">
										<i className="fa fa-thumbs-o-down" /> In-Activate
									</Dropdown.Item>
						...
						<Col md={6}>
							<ListGroup horizontal>
								<ListGroup.Item className="py-1 px-2">
									<i className="fa fa-user" /> <small>Bharani</small>
								</ListGroup.Item>
								<ListGroup.Item className="py-1 px-2">
									<i className="fa fa-calendar" /> <small>2022-03-28 22:40:43</small>
								</ListGroup.Item>
								<ListGroup.Item className="py-1 px-2">
									<i className="fa fa-clock-o" /> <small>2022-03-28 22:40:43</small>
								</ListGroup.Item>
							</ListGroup>
						</Col>
					</Row>
				...
				<Col md={3}>
					<Accordion defaultActiveKey={[ '0' ]} alwaysOpen>
						{sideMenu.map((side, i) => (
							<Accordion.Item key={i} eventKey={side.id}>
								...
								<Accordion.Body className="text-dark">
									{side.children ? (
										side.children.map((ch, j) => (
											<Accordion.Item key={j} eventKey={ch.id}>
												<Accordion.Header className="bg-light text-dark">
													{ch.label}
												</Accordion.Header>
												<Accordion.Body className="text-dark">{ch.body}</Accordion.Body>
											</Accordion.Item>
										))
									) : (
										side.body
