...
function AddPage(props) {
		...
		addAccessToForm = addAccessToForm.map((form) => {
			if (form.id === 'page_access_levels') {
				...
				const accessList = layoutContext.accessLevels.map((access) => ({
					...
					checked: String(access.accessId) === String(getSuperAdminId) ? true : false,
	...
	const onReactiveFormSubmit = () => {
		...
		const pageAccess = bData.filter((f) => f.id === 'page_access_levels')[0].value;
		const data = {
			pageLabel,
			pageRoute,
			pageAccess
	...
	return (
		<Modal {...rest} style={{ zIndex: 9999 }} backdrop="static" centered keyboard={false}>
