...
const DateTimeSelector = props => {
  ...
  const [dbMonth, setDbMonth] = useState(helpers.leadingZeros(value.getMonth() + 1));
  const [dbDate, setDbDate] = useState(helpers.leadingZeros(value.getDate()));
  const [dbHours, setDbHours] = useState(helpers.leadingZeros(value.getHours()));
  const [dbMinutes, setDbMinutes] = useState(helpers.leadingZeros(value.getMinutes()));
  const [dbSeconds, setDbSeconds] = useState(helpers.leadingZeros(value.getSeconds()));
  ...
  useEffect(() => {
    const date = new Date(dbYear, Number(dbMonth) - 1, dbDate, dbHours, dbMinutes, dbSeconds, 0);
    onChange(date);
  }, [dbYear, dbMonth, dbDate, dbHours, dbMinutes, dbSeconds]);
  return (
    <div className={`dateTimeSelector ${type === "date" ? "date" : "dateTime"}`}>
      <DateTimeDropDown
        defaultValue={dbYear}
        array={years}
