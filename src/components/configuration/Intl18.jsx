...
const Intl18 = () => {
  ...
  const master = {
    ...
    Table: "locale_master",
    label: "Locale master",
    TableRows: ["locale_id", "locale_label", "locale_string", "locale_language", "locale_currency", "locale_sort"],
    TableAliasRows: [
    ...
    rowElements: ["checkbox", "textbox", "textbox", "textbox", "textbox", "number"],
  };
  const child = {
    config: {
      header: {
  ...
  const getMaster = () => {
    setMasterData([]);
    setLoader(true);
    const a = getFromTable(master, null, apiParams.limit, apiParams.start, apiParams.searchString);
    ...
    Promise.all([a, b])
      .then(r => {
        ...
        rows?.table?.length > 0 ? setMasterData(rows) : setMasterData(defaultData);
  ...
  const getChild = () => {
    setChildData([]);
    setcLoader(true);
    getFromTable(child, `locale_ref_id = ${selectedLocaleId}`, apiChildParams.limit, apiChildParams.start, apiChildParams.searchString)
      .then(async r => {
        r?.data?.response?.table?.length > 0 ? setChildData(r.data.response) : setChildData({ table: defaultData });
      })
      .catch(() => {
        setChildData([]);
      })
        ...
        <div className='d-flex justify-content-between align-items-center'>
          <div className='d-flex align-items-center'>
            ...
            <div className='ps-2 mb-0'>
              <FormattedMessage id='internationalization' defaultMessage='internationalization' />
            </div>
          </div>
          <OffCanvas
            ...
            btnClassName={`pe-3 btn btn-sm ${userContext.userData.theme === "dark" ? "text-white" : "text-dark"}`}
            placement='end'
            key={t.id}
        ...
        {masterData?.table?.length > 0 && !loader && (
          <>
            ...
            <div className='fst-italic'>
              ...
              <ul>
                <li className='small'>
                  <FormattedMessage id='pleaseDonotEditIntlKey' defaultMessage='pleaseDonotEditIntlKey' />
                </li>
                <li className='small'>
                  <FormattedMessage id='indentOfIntlForm' defaultMessage='indentOfIntlForm' />
                </li>
              </ul>
            </div>
            <Dropdown className='pb-3'>
              <Dropdown.Toggle className='btn btn-bni'>
                {selectedLocaleId
                  ? masterData?.table?.filter(f => f.locale_id === selectedLocaleId)[0].locale_label
                  : intl.formatMessage({
                      id: "selectLanguage",
              ...
              <Dropdown.Menu>
                {masterData?.table?.map((d, i) => (
                  <Dropdown.Item onClick={() => setSelectedLocaleId(d.locale_id)} key={i}>
                    {d.locale_label}
                  </Dropdown.Item>
                ))}
              </Dropdown.Menu>
