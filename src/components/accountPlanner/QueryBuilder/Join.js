    ...
    validCriterias.forEach(object =>
      object.joins.forEach(join => {
        if (
          ...
          joins.push(
            `${object.type} JOIN ${master.tables[join.sourceFields.table]} AS ${
              join.sourceFields.table
            } ON ${join.sourceFields.table}.${join.selectedSource} = ${
              join.targetFields.table
            }.${join.selectedTarget}`,
          );
        }
      }),
          ...
          if (j === cIndex) {
            join.sourceFields =
              (master.fields[source] && {
                table: source,
                fetch: {
                  dropDownList: master.fields[source].map(
                    ({ field, marker }) => ({
                      id: field,
                      value: field,
                      marker: marker,
            ...
            join.targetFields =
              (master.fields[target] && {
                table: target,
                fetch: {
                  dropDownList: master.fields[target].map(
                    ({ field, marker }) => ({
                      id: field,
                      value: field,
                      marker: marker,
          ...
          <table
            className={`table table-borderless ${
              userContext.userData.theme === "dark"
                ? "text-white table-dark"
                : "text-dark table-light"
            }`}
          >
            <thead>
              <tr>
