...
class home_model extends CI_Model
{
  ...
  public function getUserConfig($tenantId)
  {
    ...
    $this->db
      ->select([
        ...
        'IFNULL(b.planCreditCardTrxLimit, "Infinity") as planCreditCardTrxLimit',
        'IFNULL(b.planUsersLimit, "Infinity") as planUsersLimit',
        'IFNULL(b.planCategoriesLimit, "Infinity") as planCategoriesLimit',
        'IFNULL(b.planTemplateLimit, "Infinity") as planTemplateLimit',
        ...
        'IFNULL(b.planDatasourceLimit, "Infinity") as planDatasourceLimit',
        'IFNULL(b.planWorkbookLimit, "Infinity") as planWorkbookLimit',
    ...
    return [
      ...
      "planTemplateLimit" => $row->planTemplateLimit,
      ...
      "planCreditCardTrxLimit" => $row->planCreditCardTrxLimit,
      "planStorageLimit" => $row->planStorageLimit,
      "planDatasourceLimit" => $row->planDatasourceLimit,
      "planWorkbookLimit" => $row->planWorkbookLimit,
  ...
  public function checkAppUserExists($post)
  {
    if (!empty($post["accountUserName"])) {
      $query = $this->db->where(["user_name" => strtolower($post["accountUserName"])])->get("users");
  ...
  public function onTransaction($postData, $table, $primary_field, $service = "", $appId = "", $appIdKey = "")
  {
    ...
    if (isset($postData->updateData) && count($postData->updateData) > 0) {
      ...
      if ($table === "apps") {
        if (isset($array[0]["appId"]) || isset($array[0]["name"]) || isset($array[0]["email"]) || isset($array[0]["mobile"])) {
          $this->updateCustomerInfo($array[0]["appId"], $array[0]["name"], $array[0]["email"], $array[0]["mobile"]);
        }
      }
    }
  ...
  public function signUp(array $post)
  {
    try {
      ...
      $testCustomer = $razorPayTestApi->customer
        ->create([
      ...
      $liveCustomer = $razorPayLiveApi->customer
        ->create([
          "name" => $post["accountName"],
          "email" => $post["accountEmail"],
          "fail_existing" => 0,
      ...
      $this->db->insert("users", [
        ...
        "user_name" => $userName,
        "user_display_name" => $post["accountName"],
        "user_profile_name" => $post["accountName"],
  ...
  public function getTableSize($table, $sizeColumn, $whereColumn, $id)
  {
    try {
      $query = $this->db
        ->select(["COALESCE(sum(length(" . $sizeColumn . ")),0) as size"])
        ->from($table)
