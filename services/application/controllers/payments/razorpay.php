...
class razorpay extends CI_Controller
{
  ...
  public function createSubscription()
  {
    ...
    try {
       ...
       */
      $subscription = $this->razorPayApi->subscription
        ->create([
          "plan_id" => $planId,
          "total_count" => $count,
          "customer_notify" => 1,
  ...
  public function onPaymentFailed()
  {
    ...
    if (isset($headerData["X-Razorpay-Signature"])) {
      try {
        ...
        $emailData["matter"] = [
          "<p></p>",
          '<div><b>Subscription payment failed for customer "' .
          ($payment["email"] ?? "N/A") .
          '" reachable at ' .
          ($payment["email"] ?? "N/A") .
          "</b></div>",
    ...
    if (isset($headerData["X-Razorpay-Signature"])) {
      try {
        ...
        foreach ($subscriptionActiveLists["items"] as $sub) {
          if ($sub["customer_id"] === $payment["customer_id"] && $sub["status"] === "active" && $sub["id"] !== $subscription["id"]) {
            try {
        ...
        $query = $this->db->get_where("plans", ["planId" => $plan->pricePlanId]);
        $tokenPlan = $query->row();
        ...
        $update = [
          ...
          "aiTokenSize" => $plan->priceFrequency === "month" ? $tokenPlan->planAiTokenLimit : $tokenPlan->planAiTokenLimit * 12,
        ];
