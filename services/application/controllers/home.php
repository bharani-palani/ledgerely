        ...
        if ($validate === 1) {
            ...
            if (
                ...
                $selectedAccess = array_filter(
                    $accessLevels,
                    function ($val) {
                        return $val['access_id'] === isset($post['type']);
                    }
                );
                $config = $this->home_model->getGlobalConfig();
                $appName = $config[0]['appName'];
                ...
                $emailData['saluation'] = 'Dear User,';
                $emailData['matter'] = [
                    'You have been added to ' . $appName . " app as " . $selectedAccess[0]['access_label'] . " role. ",
                    $post['userName'] . ' is your user name and ' . $post['password'] . ' is your password.',
                    'with prfile name as ' . $post['profileName'] . ' and display name as ' . $post['displayName'] . '. ',
